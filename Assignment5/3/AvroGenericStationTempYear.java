...
public class AvroGenericStationTempYear extends Configured implements Tool {
	...
	public static class AvroMapper extends
			Mapper<LongWritable, Text, AvroKey<GenericRecord>, Text> {
		...
		protected void map(LongWritable key, Text value, Context context)
			...
			if (utils.isValidTemperature()) {
				...
				context.write(new AvroKey<GenericRecord>(record), new Text(
						utils.getStationId()));
			}
		}
	}
	...
	public static class AvroReducer
			extends
			Reducer<AvroKey<GenericRecord>, Text, AvroKey<GenericRecord>, AvroValue<Text>> {
		...
		protected void reduce(AvroKey<GenericRecord> key,
			...
			if (prevYear == null) {
				prevYear = (int) key.datum().get("year");
				for (Text id : values)
					context.write(key, new AvroValue<Text>(id));
			} else if (prevYear != (int) key.datum().get("year")) {
				prevYear = (int) key.datum().get("year");
				for (Text id : values)
					context.write(key, new AvroValue<Text>(id));
			}
